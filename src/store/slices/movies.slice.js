...
export const addMovieThunk = movie => dispatch => {
    return dispatch(genericRequestThunk(async () => {
        ...
        const { data: genres } = await axios.post(`/movie/${data.id}/genres`, movie.genres);
        const { data: directors } = await axios.post(`/movie/${data.id}/directors`, movie.directors);
        const { data: actors } = await axios.post(`/movie/${data.id}/actors`, movie.actors);
...
export const updateMovieThunk = (id, movie) => dispatch => {
    dispatch(genericRequestThunk(async () => {
        const {data: movieRes} = await axios.put(`/movie/${id}`, movie)
        const { data: genres } = await axios.post(`/movie/${id}/genres`, movie.genres);
        const { data: directors } = await axios.post(`/movie/${id}/directors`, movie.directors);
        const { data: actors } = await axios.post(`/movie/${id}/actors`, movie.actors);
